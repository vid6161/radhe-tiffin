...
def init_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS business_settings (
            ...
            business_name TEXT NOT NULL DEFAULT 'Radhe Tiffin',
            ...
            pickup_policy TEXT DEFAULT '',
            cancellation_policy TEXT DEFAULT '',
            allergy_notice TEXT DEFAULT '',
    ...
    if cursor.fetchone()["count"] == 0:
        cursor.execute("""
            INSERT INTO business_settings (
                id, business_name, legal_name, owner_name, address, city,
        ...
        """, (
            ...
            'Radhe Foods',
            'Radhe Foods',
            'Viraj Desai & Saptamita Paul Choudhury',
            'Petrusplatz 3, 89231 Neu-Ulm; Bahnhofstraße 1, 89073 Ulm',
            'Neu-Ulm / Ulm',
            '89231 / 89073',
            'Germany',
            'radhefoods123@gmail.com',
            '+4917634660247',
        ))
    ...
    # --------------------------------------------------
    # CUSTOMER-REPORTED ALLERGY / INTOLERANCE REQUESTS
    # --------------------------------------------------
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS order_allergen_requests (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            order_id INTEGER NOT NULL,
            allergen_code TEXT NOT NULL,
            created_at TEXT DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (order_id) REFERENCES orders(id) ON DELETE CASCADE,
            UNIQUE(order_id, allergen_code)
        )
    """)

    # --------------------------------------------------
    # CUSTOMER CONSENTS / OPERATIONAL NOTES
    # --------------------------------------------------
    cursor.execute("PRAGMA table_info(customers)")
    customer_columns = {row["name"] for row in cursor.fetchall()}
    if "privacy_consent_at" not in customer_columns:
        cursor.execute("ALTER TABLE customers ADD COLUMN privacy_consent_at TEXT")
    if "terms_accepted_at" not in customer_columns:
        cursor.execute("ALTER TABLE customers ADD COLUMN terms_accepted_at TEXT")
    if "marketing_consent" not in customer_columns:
        cursor.execute("ALTER TABLE customers ADD COLUMN marketing_consent INTEGER DEFAULT 0")
    ...
    # --------------------------------------------------
    # INVENTORY / INGREDIENT USAGE / WASTE
    # --------------------------------------------------
    cursor.execute("""
    ...
    # Customer-reported allergens for a catering request. Catering has no fixed
    # menu at request time, so these remain a request-level safety flag rather
    # than a verified dish-level allergen match.
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS catering_allergen_requests (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            catering_request_id INTEGER NOT NULL,
            allergen_code TEXT NOT NULL,
            created_at TEXT DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (catering_request_id) REFERENCES catering_requests(id) ON DELETE CASCADE,
            UNIQUE(catering_request_id, allergen_code)
        )
    ''')
